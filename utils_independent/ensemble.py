...
def parse_args():
    ...
    # 투표 수 관련 인자
    parser.add_argument('--vote_min', type=int, default=2,
                      help='실험할 최소 투표 수 (기본값: 1)')
    parser.add_argument('--vote_max', type=int, default=4,
...
def ensemble_detections(csv_files, iou_threshold=0.5, min_vote_count=1):
    """
    UFO 형식의 여러 모델 결과를 앙상블
    
    매개변수:
        csv_files: json 파일들의 경로 리스트
        iou_threshold: 박스 중첩 판단을 위한 임계값
        min_vote_count: 박스 유지에 필요한 최소 모델 수
    """
    # 모든 결과 로드
    all_results = []
    for file_path in tqdm(csv_files, desc="파일 로딩 중"):
    ...
    for img_name in tqdm(all_image_names, desc="이미지 처리 중"):
        # 각 이미지의 모든 박스 수집
        all_boxes = []
        for model_index, result in enumerate(all_results):
            if img_name in result['images']:
                words_info = result['images'][img_name]['words']
        ...
        ensemble_result['images'][img_name] = {'words': words_info}
...
def run_experiments(args):
    ...
    # CSV 파일 경로들 수집
    csv_files = sorted(glob(os.path.join(args.input_dir, '*.csv')))
    if not csv_files:
